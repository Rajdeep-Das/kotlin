...
class InlineMetadata(val tag: JsStringLiteral, val function: FunctionWithWrapper) {
    companion object {
        ...
        fun compose(function: JsFunction, descriptor: CallableDescriptor, context: TranslationContext): InlineMetadata {
            ...
            return InlineMetadata(tag, FunctionWithWrapper(function, if (block.statements.isNotEmpty()) block else null))
        }
        ...
        fun tryExtractFunction(callExpression: JsExpression): FunctionWithWrapper? {
            ...
            return FunctionWithWrapper(function, JsBlock(callExpression.body.statements.dropLast(1)))
        }
    }

    val functionWithMetadata: JsExpression
        get() {
            val wrapperBody = function.wrapperBody
            val (markerFun, expression) = if (wrapperBody != null) {
                val wrapper = JsFunction(function.function.scope, JsBlock(), "")
                wrapper.body.statements += wrapperBody.statements
                wrapper.body.statements += JsReturn(function.function)
                Pair(Namer.createInlineFunctionBuilder(), wrapper)
            }
            else {
                Pair(Namer.createInlineFunction(), function.function)
            }

            return JsInvocation(markerFun, listOf(tag, expression))
        }
}